...
> [!NOTE]
> Prefer `computer_use_ax_tree` plus `computer_use_ax_action` for exact semantic `press` or bounded text `set_value` on a shared workstation. Vision/coordinate and keyboard synthesis use global HID and require an exclusive GUI session.
...
| Enabled non-secure macOS text fields/areas with settable `AXValue` | **Operator-safe AX** | `computer_use_ax_tree`, `computer_use_ax_action` | One-shot `set_value`, bounded to 4096 UTF-8 bytes; value never appears in the receipt; zero global-HID fallback; reinspection required. |
| Password & Sensitive Fields (Active in M10) | **Accessibility Tree only** | `computer_use_ax_tree` | Sensitive text fields display `[REDACTED]`, receive no actionable ref, and never advertise `set_value`. |
