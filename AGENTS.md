...
- **Antigravity Tooling**: Exposes the active twelve-tool MCP surface (`computer_use_status`, `computer_use_targets`, `computer_use_window_observe`, `computer_use_observe`, `computer_use_ax_tree`, `computer_use_ax_action`, `computer_use_click`, `computer_use_move`, `computer_use_type`, `computer_use_shortcut`, `computer_use_scroll`, `computer_use_drag`) alongside skill definitions compatible with Google Antigravity / Gemini 3.7 Flash. `computer_use_ax_action` is the bounded operator-safe semantic lane; legacy coordinate and keyboard tools remain global-HID surfaces, natively denied without bounded administrator-provisioned isolated-session admission (see `docs/exclusive-input.md`).
