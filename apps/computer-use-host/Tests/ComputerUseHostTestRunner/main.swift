...
public struct ComputerUseHostTestRunner {
    ...
    public static func run41_AXTreeInspectionTargetingRedactionCapsAndTruncation() async throws {
        ...
        let negativeOriginBounds = DefaultAXInspector.boundsRect(
            position: CGPoint(x: -1440, y: -143),
            size: CGSize(width: 800, height: -20)
        )
        assertEqual(negativeOriginBounds.x, -1440)
        ...
        assertEqual(semanticStatus.data?["operator_safe_ax_actions"], .array([.string("press")]))
        assertEqual(
        ...
        func issueTargetedTree(_ id: String) async -> (String, String, String) {
            ...
            if case .dictionary(let root)? = response.data?["tree"] {
                ...
                assertEqual(root["supported_actions"], .array([.string("press")]))
            } else {
        ...
        assertEqual(replay.error?.code, "STALE_AX_SNAPSHOT")
    ...
    public static func run42_MoveScrollDragPointerValidationStaleCaptureSingleUseLeaseAndUnconditionalRelease() async throws {
        // Production timing reports milliseconds, and Unicode chunks never split surrogate pairs.
        assertEqual(
            CGEventInputSynthesisEngine.durationMilliseconds(.milliseconds(1_250)),
            1_250
        )
        let unicodeText = String(repeating: "A", count: 19) + "😀B"
        let unicodeChunks = CGEventInputSynthesisEngine.utf16Chunks(for: unicodeText)
        assertEqual(unicodeChunks.map(\.count), [19, 3])
        assertEqual(
            String(decoding: unicodeChunks.flatMap { $0 }, as: UTF16.self),
            unicodeText
        )
