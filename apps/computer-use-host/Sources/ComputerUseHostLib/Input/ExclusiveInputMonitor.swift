...
package final class ExclusiveInputMonitor: @unchecked Sendable {
    private final class Context: @unchecked Sendable {
        ...
        func checkHealth() {
            ...
            if frontmost != pid { state.invalidate(reason: "target lost focus") }
    ...
    // All external events, including mouse movement, revoke exclusive authority.
    // This monitor exists only during exclusive admission; background AX is unchanged.
    ...
    package init(pid: pid_t) throws {
        ...
        guard context.started.wait(timeout: .now() + 1) == .success else {
            stop(); throw ComputerUseError.inputGuardUnavailable
        }
        context.lock.lock()
        let ready = context.ready
        context.lock.unlock()
        guard ready else { stop(); throw ComputerUseError.inputGuardUnavailable }
    }
    ...
    private func currentEpoch() -> OperatorInputEpoch {
    ...
    package func validate() throws {
        ...
        guard epoch.coverageValid else { throw ComputerUseError.inputGuardUnavailable }
        guard epoch.counters == [0] else { throw ComputerUseError.userIntervened(reason: "exclusive session external input or focus takeover") }
    }
    ...
    private static func run(_ context: Context) {
        ...
        guard let tap = CGEvent.tapCreate(tap: .cgSessionEventTap, place: .headInsertEventTap,
            options: .listenOnly, eventsOfInterest: mask, callback: { _, type, event, userInfo in
                ...
                if type == .tapDisabledByTimeout || type == .tapDisabledByUserInput {
                    context.state.invalidate()
                } else if event.getIntegerValueField(.eventSourceUnixProcessID) != Int64(getpid()) {
                    context.state.intervention(reason: "external input")
                }
        ...
        guard AXObserverAddNotification(observer, axApp, kAXApplicationDeactivatedNotification as CFString, opaque) == .success else {
            context.started.signal(); return
        }
        defer { AXObserverRemoveNotification(observer, axApp, kAXApplicationDeactivatedNotification as CFString) }
